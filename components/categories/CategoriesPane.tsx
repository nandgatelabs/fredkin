...
/** Event types list body (no app header / atmosphere). */
export function CategoriesPane({
  ...
  const reload = useCallback(async () => {
    ...
    setError(null);
    try {
      const [all, lifetime] = await Promise.all([
        listCategories(),
        getLifetimeTotals(),
      ]);
      setSections([
          ...
          renderItem={({ item }) => (
            <View style={styles.row}>
              ...
              >
                ...
                >
                  <Ionicons
                    name={categoryIcon(item.icon_key)}
                    size={18}
                    color="#fff"
                  />
                </View>
        ...
        items={[
          {
            ...
            onPress: () => {
              if (menuCategory)
                setEditor({ mode: "edit", category: menuCategory });
            },
          },
          {
            label: "Delete",
